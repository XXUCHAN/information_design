...
# 필터링된 데이터 로드
data = pd.read_csv('deposit_frequency.csv')
...
plt.bar(data['Date'], data['Frequency'], width=10, color='skyblue')
...
# 주별 빈도수 그래프
plt.figure(figsize=(12, 6))
plt.bar(weekly_data.index, weekly_data['Frequency'], width=15, color='lightgreen')
