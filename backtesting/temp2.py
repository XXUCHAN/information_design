...
# 수익률 상위 3개 결과 출력 함수 (고유 조합 기준)
def print_top_results_unique_combinations(results, top_n=5):
    seen_combinations = set()
    unique_results = []

    # 결과를 포트폴리오 가치로 정렬
    sorted_results = sorted(results, key=lambda x: x['portfolio_value'], reverse=True)

    # 고유한 조합만 선택
    for result in sorted_results:
        combination_tuple = tuple(sorted(result['combination']))  # 조합을 고유하게 정렬하여 튜플로 변환
    ...
    print(f"\nTop {top_n} Unique Results Based on Portfolio Value:")
...
def filter_results_by_strategy(results, investor_type, initial_cash=10000, top_n=3):
    # 유효한 결과 필터링: 손실보존률이 None이 아니고 기본 투자금보다 수익이 높은 경우
    valid_results = [
    ...
    if not valid_results:
        raise ValueError("No valid scenarios found with profit above the initial investment.")

    if investor_type == "defensive":
        # 손실보존률이 가장 작은 것 중 수익률이 높은 상위 3개를 선택
        ...
        # 공격적인 투자자의 경우 손실보존률이 1 미만인 결과 중 수익률이 높은 상위 3개를 선택
        aggressive_results = [res for res in valid_results if res['loss_preservation_ratio'] < 1]
        if not aggressive_results:
            raise ValueError("No valid aggressive scenarios with loss preservation ratio less than 1.")
...
try:
    ...
    print_top_results_unique_combinations(top_results, top_n=len(top_results))
