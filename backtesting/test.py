...
def print_top_results_with_details(results, merged_data, type, top_n=3, initial_cash=10000):
    ...
    if type == "aggressive":
        ...
        sorted_results = sorted(defensive_results, key=lambda x: x['loss_preservation'])
    else:
    ...
    for res in sorted_results:
        combination_tuple = tuple(sorted(res['combination']))
    ...
    print(f"Top {len(unique_results)} Results ({type.title()} Strategy):")
